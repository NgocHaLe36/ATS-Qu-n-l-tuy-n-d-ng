...
<head>
    ...
    <style>
        ...
        .admin-card { background: #fff; border: 1px solid #edf2f7; border-radius: 20px; padding: 24px; box-shadow: 0 5px 20px rgba(0,0,0,0.02); }
        
        /* Pricing Cards */
        .plan-card { background: #fff; border-radius: 24px; padding: 40px 30px; transition: 0.4s; position: relative; height: 100%; border: 2px solid transparent; box-shadow: 0 10px 30px rgba(0,0,0,0.03); display: flex; flex-direction: column; }
        .plan-card:hover { transform: translateY(-10px); box-shadow: 0 20px 40px rgba(0,123,255,0.1); }
        .plan-PRO { border: 2px solid var(--primary-blue); }
        
        .price-text { font-size: 2.5rem; font-weight: 800; color: #1e293b; }
        .price-text span { font-size: 1rem; color: #64748b; font-weight: 500; }
        
        .feature-list { list-style: none; padding: 0; margin: 30px 0; flex-grow: 1; }
        .feature-list li { margin-bottom: 12px; display: flex; align-items: flex-start; font-size: 0.9rem; color: #475569; }
        .feature-list li i { color: #10b981; margin-right: 10px; font-size: 1.1rem; }
        .feature-list li.disabled { color: #cbd5e1; text-decoration: line-through; }
        .feature-list li.disabled i { color: #cbd5e1; }

        .btn-action { border-radius: 12px; padding: 12px; font-weight: 700; width: 100%; transition: 0.3s; }
        .status-tag { position: absolute; top: 15px; right: 15px; font-size: 0.7rem; padding: 4px 12px; border-radius: 50px; font-weight: 700; text-transform: uppercase; }
    </style>
</head>
<body>
    <aside class="admin-sidebar shadow-sm">
        <%-- 1. Logo link quay về Dashboard --%>
        ...
        <ul class="sidebar-menu">
            ...
            <%-- 2. Điền đầy đủ đường dẫn cho menu Sidebar --%>
        ...
        <%-- 3. Nút Đăng xuất Sidebar đồng bộ Dashboard --%>
    ...
    <main class="admin-main">
        <header class="admin-header sticky-top shadow-sm">
            <h5 class="fw-bold mb-0 text-dark">Dịch vụ VIP</h5>
            <div class="dropdown">
        ...
        <div class="container-fluid p-4">
            <c:if test="${not empty activeSubscription}">
                <div class="admin-card mb-5 border-start border-4 border-primary">
                    <div class="d-flex justify-content-between align-items-center flex-wrap gap-3">
                        <div>
                            <h5 class="fw-bold text-dark mb-1">Gói hiện tại: <span class="text-primary">${activeSubscription.plan.name}</span></h5>
                            <p class="text-muted small mb-0">Hạn dùng đến: 
                                <b class="text-danger">
                                    ${activeSubscription.endDate.dayOfMonth}/${activeSubscription.endDate.monthValue}/${activeSubscription.endDate.year}
                                </b>
                            </p>
                        </div>
                        <div class="d-flex gap-2">
                            <form action="${pageContext.request.contextPath}/recruiter/vip/renew" method="post">
                                <input type="hidden" name="subscriptionId" value="${activeSubscription.id}">
                                <button type="submit" class="btn btn-primary fw-bold rounded-pill px-4 shadow-sm">Gia hạn ngay</button>
                            </form>
                            <form action="${pageContext.request.contextPath}/recruiter/vip/cancel" method="post" onsubmit="return confirm('Hủy gói sẽ làm mất các quyền lợi VIP sau khi hết hạn. Bạn chắc chắn?')">
                                <input type="hidden" name="subscriptionId" value="${activeSubscription.id}">
                                <button type="submit" class="btn btn-outline-danger fw-bold rounded-pill px-4">Hủy gói</button>
                            </form>
                        </div>
                    </div>
                </div>
            ...
            <div class="text-center mb-5">
                <h1 class="fw-bold text-dark">Nâng tầm thương hiệu tuyển dụng</h1>
                <p class="text-muted fs-5">Lựa chọn gói dịch vụ phù hợp để tiếp cận ứng viên tiềm năng nhanh nhất.</p>
                <c:if test="${not empty warningMessage}">
                    <div class="alert alert-warning border-0 shadow-sm d-inline-block px-4 mt-3 rounded-pill">
                        <i class="bi bi-info-circle-fill me-2"></i> ${warningMessage}
            ...
            <div class="row g-4 justify-content-center">
                <c:forEach var="plan" items="${plans}">
                    <c:set var="isActive" value="${activeSubscription != null && activeSubscription.plan.id == plan.id}" />
                    <c:set var="isHigher" value="${activeSubscription != null && plan.price > activeSubscription.plan.price}" />
                    
                    <div class="col-md-4">
                        <div class="plan-card shadow-sm plan-${plan.name.toUpperCase()}">
                            <c:if test="${isActive}">
                                <span class="status-tag bg-success text-white">Đang dùng</span>
                            </c:if>
                            ...
                            <ul class="feature-list">
                                <li><i class="bi bi-check-circle-fill"></i> Đăng tin ưu tiên lên đầu trang</li>
                                <li><i class="bi bi-check-circle-fill"></i> Làm mới tin mỗi ngày tự động</li>
                                <c:choose>
                                    <c:when test="${plan.price >= 499000}">
                                        <li><i class="bi bi-check-circle-fill"></i> <b>Huy hiệu VIP</b> xác thực tài khoản</li>
                                        <li><i class="bi bi-check-circle-fill"></i> <b>Lọc hồ sơ</b> AI nâng cao</li>
                                        <li><i class="bi bi-check-circle-fill"></i> Xem <b>SĐT ứng viên</b> trực tiếp</li>
                                    </c:when>
                                    <c:otherwise>
                                        <li class="disabled"><i class="bi bi-x-circle"></i> Huy hiệu VIP xác thực</li>
                                        <li class="disabled"><i class="bi bi-x-circle"></i> Lọc hồ sơ AI nâng cao</li>
                                    </c:otherwise>
                                </c:choose>
                                <c:choose>
                                    <c:when test="${plan.price >= 999000}">
                                        <li><i class="bi bi-check-circle-fill"></i> <b>Quảng bá logo</b> thương hiệu</li>
                                        <li><i class="bi bi-check-circle-fill"></i> <b>Hỗ trợ ưu tiên</b> 24/7</li>
                                    </c:when>
                                    <c:otherwise>
                                        <li class="disabled"><i class="bi bi-x-circle"></i> Quảng bá logo thương hiệu</li>
                                    </c:otherwise>
                                </c:choose>
                            </ul>
                            ...
                            <div class="mt-auto">
                                <c:choose>
                                    <c:when test="${isActive}">
                                        <button type="button" class="btn btn-light btn-action" disabled>Đang hoạt động</button>
                                    </c:when>
                                    <c:when test="${isHigher}">
                                        <form action="${pageContext.request.contextPath}/recruiter/vip/upgrade" method="post">
                                            <input type="hidden" name="newPlanId" value="${plan.id}">
                                            <button type="submit" class="btn btn-warning btn-action fw-bold shadow-sm">
                                                <i class="bi bi-arrow-up-circle me-1"></i> Nâng cấp ngay
                                            </button>
                                        </form>
                                    </c:when>
                                    <c:when test="${activeSubscription != null}">
                                        <button type="button" class="btn btn-light btn-action" disabled>Gói thấp hơn</button>
                                    </c:when>
                                    <c:otherwise>
                                        <form action="${pageContext.request.contextPath}/recruiter/vip/register" method="post">
                                            <input type="hidden" name="planId" value="${plan.id}">
                                            <button type="submit" class="btn btn-primary btn-action shadow-sm">Đăng ký ngay</button>
                                        </form>
                                    </c:otherwise>
                                </c:choose>
                            </div>
