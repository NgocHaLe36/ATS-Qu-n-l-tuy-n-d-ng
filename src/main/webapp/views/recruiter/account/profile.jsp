...
<head>
    ...
    <title>Hồ sơ công ty - ATS Recruiter</title>
    ...
    <style>
        ...
        .sidebar-link { display: flex; align-items: center; padding: 12px 15px; color: #555; text-decoration: none; font-weight: 500; border-radius: 10px; transition: 0.3s; }
        ...
        .profile-avatar-lg { width: 100px; height: 100px; border-radius: 20px; object-fit: cover; border: 3px solid #fff; box-shadow: 0 5px 15px rgba(0,0,0,0.05); }
        
        .info-label { font-size: 0.75rem; color: #94a3b8; font-weight: 700; text-transform: uppercase; margin-bottom: 8px; display: block; }
        .form-control-custom { border-radius: 10px; border: 1px solid #e2e8f0; padding: 10px 15px; font-weight: 500; }
        .form-control-readonly { background-color: #f8fafc; color: #64748b; border: 1px solid #e2e8f0; cursor: not-allowed; }
        
        .status-badge { padding: 4px 12px; border-radius: 50px; font-size: 0.75rem; font-weight: 600; }
        .vip-card { background: linear-gradient(135deg, #007bff 0%, #0056b3 100%); color: white; border: none; }
...
<body>
    ...
    <aside class="admin-sidebar shadow-sm">
        ...
        <ul class="sidebar-menu">
            ...
            <%-- 2. Cập nhật các link chức năng Sidebar --%>
            ...
            <li class="sidebar-item">
                <a href="${pageContext.request.contextPath}/recruiter/account/profile" class="sidebar-link active">
                    <i class="bi bi-person-gear"></i> Hồ sơ công ty
                </a>
            </li>
    ...
    <main class="admin-main">
        <header class="admin-header sticky-top shadow-sm d-flex justify-content-between align-items-center">
            <h5 class="fw-bold mb-0 text-dark">Hồ sơ công ty</h5>
            <div class="dropdown">
        ...
        <div class="container-fluid p-4">
            ...
            <form action="${pageContext.request.contextPath}/recruiter/account/profile" method="POST">
                <div class="admin-card">
                    <div class="row align-items-center">
                        <div class="col-auto">
                            <img src="https://ui-avatars.com/api/?name=${recruiter.fullName}&background=random" class="profile-avatar-lg">
                        </div>
                        <div class="col">
                            <h3 class="fw-bold mb-1">${recruiter.fullName}</h3>
                            <div class="d-flex align-items-center gap-2 mb-3 text-muted small">
                                <span><i class="bi bi-envelope me-1"></i> ${recruiter.email}</span>
                                <c:if test="${not empty activeSubscription}">
                                    <span class="status-badge bg-warning bg-opacity-10 text-warning ms-2">ĐỐI TÁC VIP</span>
                                </c:if>
                            </div>
                ...
                <div class="row g-4">
                    <div class="col-lg-8">
                        <div class="admin-card h-100">
                            <h5 class="fw-bold mb-4 border-bottom pb-2">Thông tin chi tiết</h5>
                            <div class="row g-3">
                                <div class="col-md-6">
                                    <label class="info-label">Tên người đại diện</label>
                                    <input type="text" name="fullName" class="form-control form-control-custom" value="${recruiter.fullName}" required>
                                </div>
                                <div class="col-md-6">
                                    <label class="info-label">Email liên hệ</label>
                                    <input type="email" name="email" class="form-control form-control-custom" value="${recruiter.email}" required>
                                </div>
                                <div class="col-md-6">
                                    <label class="info-label">Vai trò</label>
                                    <input type="text" class="form-control form-control-custom form-control-readonly" value="${recruiter.role}" readonly>
                                </div>
                                <div class="col-md-6">
                                    <label class="info-label">Mã tài khoản</label>
                                    <input type="text" class="form-control form-control-custom form-control-readonly" value="#REC-${recruiter.id}" readonly>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="col-lg-4">
                        <div class="admin-card vip-card h-100">
                            <h5 class="fw-bold mb-3 border-bottom border-white border-opacity-25 pb-2">Dịch vụ hiện tại</h5>
                            <c:choose>
                                <c:when test="${not empty activeSubscription}">
                                    <h4 class="fw-bold mb-1">${activeSubscription.plan.name}</h4>
                                    <p class="small opacity-75 mb-3">
                                        Hết hạn: ${activeSubscription.endDate}
                                    </p>
                                    <a href="${pageContext.request.contextPath}/recruiter/vip/plans" class="btn btn-light btn-sm w-100 fw-bold">Gia hạn gói</a>
                                </c:when>
                                <c:otherwise>
                                    <p class="small mb-3 opacity-75">Bạn hiện đang sử dụng gói Miễn phí.</p>
                                    <a href="${pageContext.request.contextPath}/recruiter/vip/plans" class="btn btn-warning btn-sm w-100 fw-bold text-dark shadow-sm">Nâng cấp VIP ngay</a>
                                </c:otherwise>
                            </c:choose>
                        </div>
                    </div>
